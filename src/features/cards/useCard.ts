...
import { listReelIdsForCard } from "@/db/repositories/reelsRepository";
  ...
  const reload = useCallback(async () => {
    ...
    try {
      if (!db) {
        ...
        setLoadError(
          databaseError ??
            new Error("Native database is not available on this platform."),
        );
        return;
      }

      const [nextCard, nextAssets, nextTags, nextReelIds] = await Promise.all([
        getCardById(db, cardId),
        listAssetsForCard(db, cardId),
        listTagsForCard(db, cardId),
        listReelIdsForCard(db, cardId),
      ]);

      setCard(nextCard);
      setAssets(nextAssets);
      setTags(nextTags);
      setReelIds(nextReelIds);
